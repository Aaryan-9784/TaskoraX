        ...
        <div className="bg-surface-secondary/50 rounded-3xl p-6 border border-border/40 flex flex-col sm:flex-row items-center gap-6 shadow-sm hover:shadow-md transition-shadow duration-300">
          <div className="relative group">
            <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-primary-100 to-primary-50 flex items-center justify-center text-primary-600 text-3xl font-bold overflow-hidden border-4 border-white shadow-lg ring-4 ring-primary-50/50">
              {form.avatar ? (
                <img src={form.avatar} alt="Profile" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
              ) : (
                form.name.charAt(0).toUpperCase() || 'U'
              )}
            </div>
            <button type="button" onClick={handlePictureClick} className="absolute bottom-0 right-0 p-2 bg-white border border-border rounded-full text-text-secondary hover:text-primary-600 hover:border-primary-200 transition-all shadow-md hover:scale-110 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2">
          ...
          <div className="text-center sm:text-left">
            ...
            <div className="flex items-center justify-center sm:justify-start gap-3">
              <Button type="button" variant="secondary" size="sm" onClick={handlePictureClick}>Change Picture</Button>
              <button type="button" onClick={handleRemovePicture} className="text-sm text-danger-500 hover:text-danger-600 font-medium transition-colors">Remove</button>
